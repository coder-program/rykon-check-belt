...
import { Building2, Users, TrendingUp, Activity, Award, ChevronDown, MapPin, DollarSign, Calendar } from 'lucide-react';
...
export default function DashboardMultiUnidade({ onSelectUnidade }: Props) {
  ...
  const [unidadesData, setUnidadesData] = useState<UnidadeData[]>(mockUnidadesData);
  ...
  useEffect(() => {
    ...
    const totais = unidadesData.reduce((acc, unidade) => ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-blue-200 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-gray-900">Visão Multi-Unidade</h2>
          </div>
          ...
          <div className="relative">
            ...
            >
              ...
              <span className="font-medium text-gray-700">{getUnidadeNome()}</span>
                  ...
                  <div className="p-2">
                    <button
                      ...
                      className={`w-full text-left px-4 py-2 rounded-lg hover:bg-blue-50 transition-colors ${
                        selectedUnidade === 'todas' ? 'bg-blue-100 text-blue-700' : ''
                      }`}
                    >
                      <div className="font-medium">Todas as Unidades</div>
                      <div className="text-xs text-gray-500">Visão consolidada da rede</div>
                    </button>
                    ...
                    {unidadesData.map(unidade => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-2 rounded-lg hover:bg-blue-50 transition-colors ${
                          selectedUnidade === unidade.id ? 'bg-blue-100 text-blue-700' : ''
                        }`}
                      >
                        <div className="font-medium">{unidade.nome}</div>
                        <div className="text-xs text-gray-500">{unidade.endereco}</div>
                      </button>
                    ))}
                  </div>
                </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold">{dadosExibicao.totalAlunos}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold">{dadosExibicao.presencasHoje}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">R$ {(dadosExibicao.receitaMes / 1000).toFixed(0)}k</p>
              </div>
        ...
        <Card className={`${dadosExibicao.crescimento >= 0 ? 'bg-gradient-to-br from-emerald-500 to-teal-600' : 'bg-gradient-to-br from-red-500 to-red-600'} text-white`}>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-white/90 text-sm">Crescimento</p>
                <p className="text-3xl font-bold">{dadosExibicao.crescimento > 0 ? '+' : ''}{dadosExibicao.crescimento.toFixed(1)}%</p>
              </div>
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <div className="text-center">
                        <p className="text-2xl font-bold text-green-600">{unidade.presencasHoje}</p>
                        <p className="text-xs text-gray-500">Presenças</p>
                      </div>
                      
                      <div className="text-center">
                        <p className="text-xl font-bold text-yellow-600">R$ {(unidade.receitaMes / 1000).toFixed(0)}k</p>
                        <p className="text-xs text-gray-500">Receita/mês</p>
                      </div>
                      
                      <div className="text-center">
                        <p className={`text-xl font-bold ${unidade.crescimento >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
      ...
      {selectedUnidade === 'todas' && (
        <Card className="bg-white border border-blue-200">
          ...
          <CardContent>
            <div className="space-y-4">
              {unidadesData.map(unidade => {
                const percentualAlunos = (unidade.totalAlunos / totaisRede.totalAlunos) * 100;
                const percentualReceita = (unidade.receitaMes / totaisRede.receitaTotal) * 100;
                
                return (
                  <div key={unidade.id} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium text-gray-700">{unidade.nome}</span>
                      <span className="text-xs text-gray-500">{unidade.totalAlunos} alunos</span>
                    </div>
                    
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-gray-500 w-16">Alunos:</span>
