...
export class PerfisService {
  ...
  async create(createPerfilDto: CreatePerfilDto): Promise<Perfil> {
    ...
    if (createPerfilDto.permissao_ids && createPerfilDto.permissao_ids.length > 0) {
  ...
  async update(id: string, updateData: Partial<CreatePerfilDto>): Promise<Perfil> {
  ...
  async removePermissao(perfilId: string, permissaoId: string): Promise<Perfil> {
    const perfil = await this.findOne(perfilId);
