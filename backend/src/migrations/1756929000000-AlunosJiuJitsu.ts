...
export class AlunosJiuJitsu1756929000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // Verificar se a tabela alunos já existe
    ...
    `);
      ...
      await queryRunner.query(
        `CREATE INDEX idx_alunos_nome ON teamcruz.alunos(nome_completo)`,
      );
      await queryRunner.query(
        `CREATE INDEX idx_alunos_cpf ON teamcruz.alunos(cpf)`,
      );
      await queryRunner.query(
        `CREATE INDEX idx_alunos_unidade ON teamcruz.alunos(unidade_id)`,
      );
      await queryRunner.query(
        `CREATE INDEX idx_alunos_status ON teamcruz.alunos(status)`,
      );
      await queryRunner.query(
        `CREATE INDEX idx_alunos_faixa ON teamcruz.alunos(faixa_atual)`,
      );
      await queryRunner.query(
        `CREATE INDEX idx_alunos_matricula ON teamcruz.alunos(numero_matricula)`,
      );
    } else {
    ...
    await queryRunner.query(`
      ...
      BEGIN
        IF NOT EXISTS (
          ...
          AND tgrelid = 'teamcruz.alunos'::regclass
        ) THEN
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    // Remover triggers
    await queryRunner.query(
      `DROP TRIGGER IF EXISTS gerar_matricula_aluno ON teamcruz.alunos`,
    );
    await queryRunner.query(
      `DROP TRIGGER IF EXISTS update_alunos_updated_at ON teamcruz.alunos`,
    );

    // Remover funções
    await queryRunner.query(
      `DROP FUNCTION IF EXISTS teamcruz.gerar_numero_matricula()`,
    );
    await queryRunner.query(
      `DROP FUNCTION IF EXISTS teamcruz.update_updated_at_column()`,
    );

    // Remover índices
    await queryRunner.query(
      `DROP INDEX IF EXISTS teamcruz.idx_alunos_matricula`,
    );
